...
impl OAuth2ChallengeRepo {
    ...
    pub async fn insert_challenge(
        ...
        let challenge = sqlx::query_as::<_, OAuth2Challenge>(
            "SELECT * FROM oauth2_challenges WHERE csrf_token = ?1",
        )
        .bind(csrf_token)
        .fetch_one(&self.db)
        .await?;
    ...
    pub async fn get_challenge(&self, csrf_token: &str) -> anyhow::Result<OAuth2Challenge> {
        let challenge = sqlx::query_as::<_, OAuth2Challenge>(
            "SELECT * FROM oauth2_challenges WHERE csrf_token = ?1",
        )
        .bind(csrf_token)
    ...
    pub async fn delete_challenge(&self, csrf_token: &str) -> anyhow::Result<()> {
